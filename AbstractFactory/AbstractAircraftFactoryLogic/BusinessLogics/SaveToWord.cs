...
namespace AbstractAircraftFactoryLogic.BusinessLogics
{
	static class SaveToWord
	{
		...
		public static void CreateDoc(WordInfo info)
		{
			using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
			{
				...
				foreach (var aircraft in info.Aircrafts)
				{
					...
					{
						Texts = new List<(string, WordTextProperties)> {
							(aircraft.AircraftName, new WordTextProperties { Bold = true, Size = "24", }),
							(" Цена: " + aircraft.Price.ToString(), new WordTextProperties
							{ Bold = false, Size = "24", })
						},
		...
		public static void CreateDocStorage(WordInfoStorage info)
		{
			...
			using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
			{
				...
				foreach (var storage in info.Storages)
				{
					...
					cellPerson.Append(new TableCellProperties(
						new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
					cellPerson.Append(new Paragraph(new Run(new Text(storage.ResponsiblePerson))));

					TableCell cellDateCreate = new TableCell();
					cellDateCreate.Append(new TableCellProperties(
						new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
					cellDateCreate.Append(new Paragraph(new Run(new Text(storage.DateCreate.ToString()))));
